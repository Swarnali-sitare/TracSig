...
export const Header = ({ onMenuClick }: HeaderProps) => {
  ...
  const notifications = [
    { id: 1, text: "New assignment posted in CS101", time: "5 min ago", unread: true },
    { id: 2, text: "Assignment due in 2 days", time: "1 hour ago", unread: true },
    { id: 3, text: "Your submission was evaluated", time: "3 hours ago", unread: false },
  ];
              ...
              <div className="absolute right-0 mt-2 w-80 bg-white rounded-lg shadow-lg border border-[rgba(0,0,0,0.1)] overflow-hidden">
                ...
                <div className="max-h-96 overflow-y-auto">
                  {notifications.map((notification) => (
                    <div
                      key={notification.id}
                      className={`p-4 border-b border-[rgba(0,0,0,0.1)] hover:bg-[#F8FAFC] cursor-pointer ${
                        notification.unread ? "bg-[#EEF2FF]" : ""
                      }`}
                    >
                      <p className="text-sm text-[#1F2937]">{notification.text}</p>
